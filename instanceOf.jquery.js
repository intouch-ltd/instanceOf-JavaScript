		 ...
		 instanceOf : function( objectRef, className ){
			...
			if(jqueryTypeName !== 'undefined'){
				if(jqueryTypeName !== 'null'){
					if($.type(className) === 'string'){ //TODO this will not allow for a String object to be passed
						var classPath = className.split('.'); //allows use of namespacing
						if(jqueryTypeName.toLowerCase() !== "object"){
							return jqueryTypeName.toLowerCase() === className.toLowerCase();
						} else {
